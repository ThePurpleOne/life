...
// Any dead cell with exactly three live neighbours becomes a live cell, as if by reproduction.
	...
	for (uint64_t x = 0; x < w->w; x++)
	{
		for (uint64_t y = 0; y < w->h; y++)
		{
			...
			// Change States
			if( pc->active && (n < 2) ) {pc->active = false; killed++;} 					// Underpopulation
			else if( pc->active && (n > 3) ){pc->active = false; killed++;} 				// Overpopulation
			else if( pc->active && ((n == 2) || (n == 3)) ){pc->active = true;kept++;} 		// Normal
			else if( (pc->active == false) && (n == 3)){pc->active = true;spawned++;} 		// Reproduction
		}
	}
